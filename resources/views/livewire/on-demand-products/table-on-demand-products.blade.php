                ...
                <div class="flex flex-col gap-x-3 gap-y-2 sm:flex-row md:ml-auto">
                    ...
                    @can('articles.create')
                        <x-base.button
                            class="group-[.mode--light]:!border-transparent group-[.mode--light]:!bg-white/[0.12] group-[.mode--light]:!text-slate-200"
                            variant="primary"
                            onclick="window.location.href='{{ route('on-demand-products.create') }}'"
                        >
                            <div class="px-2"><i class="fa-solid fa-plus"></i></div>
                            Nuevo producto a pedido
                        </x-base.button>
                    @endcan
                    <x-base.button
                        class="group-[.mode--light]:!border-transparent group-[.mode--light]:!bg-white/[0.12] group-[.mode--light]:!text-slate-200"
                        variant="primary"
                        wire:click="reportOnDemand"
                    >
            ...
            <div class="mt-3.5">
                <div class="box box--stacked flex flex-col">
                    <div class="flex flex-col gap-y-2 p-5 sm:flex-row sm:items-center justify-end">
                        ...
                        <div>
                            <div class="relative mr-2">
                                ...
                                >
                                    ...
                                    <option value="asc">A-Z Producto</option>
                                    <option value="desc">Z-A Producto</option>
                                </x-base.form-select>
                            </div>
                        </div>
                        <div>
                        ...
                        <div>
                            <div class="relative">
                                <i class="absolute inset-y-0 left-0 z-10 my-auto ml-3.5 h-4 w-4 stroke-[1.3] text-slate-500 fa-solid fa-percent"></i>
                                <x-base.form-select class="rounded-[0.5rem] pl-9 sm:w-64"
                                                    wire:model.live="sortMargin">
                                    <option value="">Ordenar por Margen</option>
                                    <option value="asc">Menor margen</option>
                                    <option value="desc">Mayor margen</option>
                                </x-base.form-select>
                            </div>
                        </div>
                    </div>
                        ...
                        <x-base.table class="border-b border-slate-200/60">
                            <x-base.table.thead>
                                <x-base.table.tr>
                                    ...
                                    <x-base.table.td class="w-28 border-t border-slate-200/60 bg-slate-50 py-4 font-medium text-slate-500">
                                        Stock
                                    </x-base.table.td>
                                    @if(auth()->user()->id == 1)
                                        <x-base.table.td class="w-36 border-t border-slate-200/60 bg-slate-50 py-4 font-medium text-slate-500">
                                            Precio Compra
                                        </x-base.table.td>
                                    @endif
                                    <x-base.table.td class="w-36 border-t border-slate-200/60 bg-slate-50 py-4 font-medium text-slate-500">
                            ...
                            <x-base.table.tbody>
                                @if($articles->count() > 0 )
                                    @foreach ($articles as $article)
                                        <x-base.table.tr class="[&_td]:last:border-b-0">
                                            ...
                                            <x-base.table.td class="border-dashed py-4 dark:bg-darkmode-600">
                                                @if($article->stock == 0)
                                                    <span class="bg-gray-100 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-gray-300">{{$article->stock}}</span>
                                                ...
                                                @elseif($article->stock > 10)
                                                    <span class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-300">{{$article->stock}}</span>
                                                @endif
                                            </x-base.table.td>
                                            @if(auth()->user()->id == 1)
                                                <x-base.table.td class="border-dashed py-4 dark:bg-darkmode-600">
                                            ...
                                            <x-base.table.td class="border-dashed py-4 dark:bg-darkmode-600">
                                                @php
                                                    ...
                                                    $cost   = (float) $article->purchase_price * (float) $rate;
                                                    $profit = $sale - $cost;
                                ...
                                @else
                                    <x-base.table.tr>
                                        <x-base.table.td colspan="11" class=" text-center border-dashed py-4 dark:bg-darkmode-600">
